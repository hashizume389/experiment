...
class MoveSquareNode(Node):
    """
    SCOUT MINIが「前進 -> 90度時計回り」を4回繰り返し、
    正方形の軌道を描くノード。
    最終的に初期位置・姿勢に戻る動作：
      前進1 → 右折1 → 前進2 → 右折2 → 前進3 → 右折3 → 前進4 → 右折4 → 完了
    ...
    def __init__(self):
        ...
        self.max_loops_ = 4           # ループ回数 (前進+右折のセット数)
        
        self.linear_velocity_ = 0.1    # 前進速度 (m/s)
        self.angular_velocity_ = 0.1   # 回転速度 (rad/s) (正の値)
        
        self.odom_topic_ = '/odom'
        self.vslam_topic_ = '/visual_slam/tracking/odometry'  # Visual SLAM オドメトリ
        self.cmd_vel_topic_ = '/cmd_vel'
        
        # Visual SLAMオドメトリ用の変数
        self.vslam_position_ = None
        self.vslam_yaw_ = 0.0
        ...
        self.loop_count_ = 0  # 完了した「前進+右折」のセット数
        ...
        try:
            ...
            # ヘッダー: w_ がWheel Odometry, v_ がVisual SLAM
            self.csv_writer_.writerow([
                'timestamp',
                'w_x', 'w_y', 'w_z', 'w_yaw',      # Wheel Odometry (制御に使用)
                'v_x', 'v_y', 'v_z', 'v_yaw'       # Visual SLAM (参照用)
            ])
        ...
        # サブスクライバー 1: Wheel Odometry (制御メイン)
        ...
        self.get_logger().info(
            f'ノード起動: [前進 -> 90度右折] x 4回 を実行します...'
        )
        self.get_logger().info(f'オドメトリトピック [{self.odom_topic_}] を待機中...')
        self.get_logger().info(f'Visual SLAMトピック [{self.vslam_topic_}] も記録します。')
    ...
    def vslam_callback(self, msg: Odometry):
        """Visual SLAMオドメトリのデータを受信して保持するだけ"""
        self.vslam_position_ = msg.pose.pose.position
        q = msg.pose.pose.orientation
        t3 = +2.0 * (q.w * q.z + q.x * q.y)
        t4 = +1.0 - 2.0 * (q.y * q.y + q.z * q.z)
        self.vslam_yaw_ = math.atan2(t3, t4)
    ...
    def odom_callback(self, msg: Odometry):
        ...
        if self.csv_writer_:
            try:
                ...
                # Visual SLAMデータがまだ来ていない場合のガード
                vx = self.vslam_position_.x if self.vslam_position_ else 0.0
                vy = self.vslam_position_.y if self.vslam_position_ else 0.0
                vz = self.vslam_position_.z if self.vslam_position_ else 0.0
                vyaw = self.vslam_yaw_
    ...
    def timer_callback(self):
        ...
        動作シーケンス: 前進1 → 右折1 → 前進2 → 右折2 → 前進3 → 右折3 → 前進4 → 右折4 → 完了
        """
        if self.initial_position_ is None:
        ...
        # --- 状態 2: 90度時計回り ---
            ...
            if self.angle_traveled_ > (self.target_angle_ + self.rotation_tolerance_):
                ...
                self.get_logger().info(
                    f'回転中 ({self.loop_count_ + 1}/{self.max_loops_}回目): {self.angle_traveled_:.2f}rad / {self.target_angle_:.2f}rad',
                    throttle_duration_sec=1
                )
            else:
                # 目標達成: カウンターを増やし、次の状態へ
                self.loop_count_ += 1
                self.get_logger().info(f'目標角度 {self.target_angle_:.2f}rad に到達。({self.loop_count_}/{self.max_loops_})')
                
                if self.loop_count_ >= self.max_loops_:
                    # 4セット完了（前進4回 + 右折4回）
                    self.current_state_ = self.STATE_DONE
                    self.get_logger().info('状態遷移: 完了 (DONE)')
                    self.get_logger().info('全てのシーケンスが完了しました。初期位置・姿勢に戻りました。')
                    self.close_csv_file()
                else:
                    # 次の前進へ
                    self.current_state_ = self.STATE_MOVING_FORWARD
